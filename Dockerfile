...
RUN chown -R developer:developer /.devstep && \
    ...
    ln -s /.devstep/bin/create-cache-symlinks /etc/my_init.d/001-create-cache-symlinks.sh && \
    ln -s /.devstep/bin/fix-permissions /etc/my_init.d/002-fix-permissions.sh && \
